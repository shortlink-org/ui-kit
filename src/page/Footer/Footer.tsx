...
export interface FooterLink {
  ...
  /** Link text */
  label: string
  /** Link URL - required and must be non-empty */
  href: string
  /** Open in new tab */
  target?: '_blank' | '_self'
...
export interface SocialLink {
  /** Social network name (for accessibility) */
  name: string
  /** Link URL */
  href: string
  /** SVG icon path */
  iconPath: string
  /** Icon viewBox (default: "0 0 1792 1792") */
  viewBox?: string
}
...
export interface FooterProps {
  ...
  /** Copyright text or component */
  copyright?: ReactNode
  /** Logo slot - rendered at the top of footer */
  logoSlot?: ReactNode
  /** Description text - rendered below logo */
...
export const Footer: React.FC<FooterProps> = ({
  ...
  const renderLink = (link: FooterLink) => {
    // Validate href - must be non-empty
      ...
      <div className="max-w-screen-xl mx-auto px-4">
        ...
        <ul className="max-w-screen-md mx-auto text-lg font-light flex flex-wrap justify-between dark:prose-dark dark:md:prose-xl-dark items-center">
          {links.map((link, index) => {
            ...
            if (!renderedLink) return null
            return (
              <li key={linkId} className="my-2 cursor-pointer">
                {renderedLink}
              </li>
            )
          })}
        ...
        <div className="pt-8 flex max-w-xs mx-auto items-center justify-between">
          {socialLinks.map((social, index) => {
            const socialId = social.href || `social-${index}`
            return (
              <a
                key={socialId}
                href={social.href}

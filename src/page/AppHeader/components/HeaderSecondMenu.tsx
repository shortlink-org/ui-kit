...
export function HeaderSecondMenu({ items, label, LinkComponent }: HeaderSecondMenuProps) {
  const renderLink = (href: string, children: React.ReactNode, linkClassName?: string) => {
    if (LinkComponent) {
      return (
                ...
                return (
                  <div key={itemKey} className="group relative flex gap-x-6 rounded-lg p-4 hover:bg-white/5">
                    ...
                    {!IconComponent && item.icon && (
                      <div className="mt-1 flex size-11 flex-none items-center justify-center rounded-lg bg-gray-700/50 group-hover:bg-gray-700">
                        <span className="text-lg text-gray-400 group-hover:text-white">{item.icon}</span>
                      </div>
                    )}
                    <div>
                      {item.href ? (
